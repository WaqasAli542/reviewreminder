...
namespace WMZ\ReviewReminder\Model\Repository;
...
use WMZ\ReviewReminder\Api\Data\ReviewReminderInterfaceFactory;
use WMZ\ReviewReminder\Api\ReviewReminderRepositoryInterface;
use WMZ\ReviewReminder\Model\ResourceModel\ReviewReminder;
...
class ReviewReminderRepository implements ReviewReminderRepositoryInterface
{
     ...
     */
    private $reviewReminderResultsFactory;
    ...
    {
        $reminder=$this->reviewReminderFactory->create();
        $this->resourceModel->load($reminder, $orderId, ReviewReminderInterface::ORDER_ID);
        if (!$reminder->getReminderId()) {
            throw new NoSuchEntityException(__('Order with specified ID "%1" not found.', $orderId));
        }
        return $reminder;
    }
}
