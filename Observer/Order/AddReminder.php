...
namespace WMZ\ReviewReminder\Observer\Order;
...
class AddReminder implements ObserverInterface
{
     ...
     */
    private $reviewReminderRepository;

    /**
     * @var ReviewReminderInterfaceFactory
     */
    private $reviewReminderInterfaceFactory;
        ...
        if ($order->getStatus() == $this->helperData->getOrderStatus()) {
            ...
            if ($canProcessOrder) {
                try {
                    ...
                    $reminder->setStatus(\WMZ\ReviewReminder\Model\Config\Source\Status::NOT_SEND);
